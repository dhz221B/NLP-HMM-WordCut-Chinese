...
def text_to_state(file_text, file_state):  # 将原始的语料库转换为 对应的状态文件
    ...
    with open(file_state, "w", encoding="utf-8") as f:    #  代开写入的文件
        for d_index, data in tqdm(enumerate(all_data)):              # 逐行遍历, tqdm 是进度条提示, data 是一篇文章, 有可能为空
            if data:                                                 #  如果 data 不为空
                state_ = ""
...
class HMM:
    ...
    def test(self, file_test, file_result):
        all_data = open(file_test, "r", encoding="utf-8").read().split("\n")
